...
// get your friends
app.get('/api/friends', protectRoute, async (req, res) => {
    const userId = req.user.id;

    const fiendDetailsModel = {
        id: true,
        username: true,
        status: true, // might be irrelevant because i can get it from redis map
        profilePictureUrl: true,
        fullName: true,
    };

    try {
        // fetch all friends where user is either userA or userB
        const rawFriendList = await prisma.friendship.findMany({
            where: {
                OR: [{ userId: userId }, { friendId: userId }],
            },
            include: {
                user: {
                    select: fiendDetailsModel,
                },
                friend: {
                    select: fiendDetailsModel,
                },
